...
- For audio: **oMLX** running locally with **Qwen3-TTS-12Hz-1.7B-CustomVoice-8bit**, voice **aiden** (`omlx start`, default `http://127.0.0.1:8000`)
- Optional: `ffmpeg` if oMLX returns WAV instead of MP3

The first audio run takes several minutes (~480 clips). Later runs only build new hashes. Clip ids include the TTS model, voice, style instruction, and temperature, so changing any of those regenerates audio.
...
| Different TTS model / voice / server | `OMLX_TTS_MODEL` / `OMLX_TTS_VOICE` / `OMLX_BASE_URL` (or the constants in `build_booklet.py`) |
